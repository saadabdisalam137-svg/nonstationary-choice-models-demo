...
OUT_DIR   = "/Volumes/SacadsProjects/ProjectAutism/ModelParameterAndBiasEst."
...
for idx, fname in enumerate(files):
    ...
    # 2) point loader to YOUR exact file list and re-assert the index (belt & suspenders)
    data_extractor.links = files[:]   # use this list for load_data/get_learning_phases
    data_extractor.mouse_num = idx

    # 3) run estimator in real mode; empty lists => auto-load via loader
    mm = ModelParameterEstimatesFromSAR_rev(
        realOrSim="real",
        l=400, w=1,  # adjust window/step if needed
        full_states=[], full_actions=[], full_rewards=[],
        reversal_trials=[],            # phases come from loader.get_learning_phases()
        protocol="rev",
        mouse_type="WT",
    ...
    # 4) save per-mouse CSV (safer on network/external drives)
    try:
    ...
    out_csv = os.path.join(OUT_DIR, f"{tag}_parameter_estimates.csv")
    params_to_df(ALPHAS, BETAS, BL, BR, BN, COLORS).to_csv(out_csv, index=False)
